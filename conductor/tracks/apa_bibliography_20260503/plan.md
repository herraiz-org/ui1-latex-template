...
- [ ] Task: Diagnose the existing `make` error.
    - [ ] Run `make clean` and `make` to reproduce the compilation error.
    - [ ] Analyze the logs (e.g., `plantilla.log`, `plantilla.blg`) to identify the root cause of the bibliography failure.
- [ ] Task: Update or create tests for the bibliography build.
    - [ ] Ensure a test LaTeX file exists that specifically uses APA citations to verify the backend and localization.
    - [ ] Run the compilation on the test file to confirm it currently fails (Red Phase).
